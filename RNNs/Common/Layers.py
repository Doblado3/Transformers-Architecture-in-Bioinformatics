...
class Dense:
    def __init__(self, input_size, output_size, activate_func=None, deriv_activate_func=None, activate_class=None):
        #self es para indicar que es la instancia de esta clase
        self.W = np.random.uniform(size=(output_size, input_size)) #Generamos pesos y sesos aleatorios en la primera ronda
    ...
    def num_params(self):
            return self.W.size + self.b.size #Dependerán del número de nodos de cada capa
    ...
    def back_prop(self, grads):
            self.dW = np.zeros_like(self.W)#Initializes self.dW and self.db to have the same shape as self.W and self.b
            self.db = np.zeros_like(self.b)#ensuring they are ready to store the updatings of backpropagation
            ...
            return np.dot(self.W.T, self.db) #Devolvemos pesos y sesgo actualizados.
...
class Conv2D:
    ...
    def forward_prop(self, image):
        ...
        #Inicialización de las dimensiones
        h, w, _= image.shape
        #Creamos el "almacen" de los resultados de la convolución
        ...
        #Una vez que hemos aplicado los kernels sobre todos los canales,
        #sumamos eñl resultado de todos los canales para obtener una sola salida por kernel en cada posición            
