...
#Código para construir una RNN from scratch, creando el dataset, entrenándola y viendo como funciona
...
class SimpleRNN:
    def __init__(self, input_units, hidden_units, activate_func, deriv_activate_func, return_sequences=False):
        ...
        #Inicializamos los pesos con los algoritmos descritos arriba
    ...
    def forward_prop(self, x, n_sequence):
        ...
        for t in range(self.n_sequence):
            ...
            if t > 0:
                self.h_h[t] += np.dot(self.U, self.activate_func(self.h_h[t - 1])) #Usamos el hidden state anterior
    ...
    #En esta función el primer bucle calcula los gradientes del hidden state
    #mientras que el segundo calcula los del resto de parámetros, en base a los del hidden state
...
# Create dataset
# ============================
n_sequence = 25 #25 puntos previos al target
n_data = 100 #100 puntos totales de curva
...
# ============================
# Create Model
# Una RNN simple, con tanh como función de activación
# y una Dense Layer haciendo de output con una función de activación linear
# ============================
