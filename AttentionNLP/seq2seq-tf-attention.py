#
#Traductor de Español a Inglés usando un modelo seq2seq con Attention
#
...
import logging
import time
import sys #maneja argumentos de la línea de comandos
sys.path.append("..")
from Common import LanguageTranslationHelper as lth

logging.getLogger("tensforflow").setLevel(logging.ERROR) #suppress warnings
...
#
#Seleccionamos el tipo de Attention a implementar
#
...
#
#Definimos tanto el encoder como el decoder
#
...
class Encoder(tf.keras.Model):
    def __init__(self, vocab_size, embedding_dim, enc_units, batch_sz):
        ...
        """
        A diferencia de el paper original, usaremos una GRU simple como Encoder
        ...
        self.gru = tf.keras.layers.GRU(
            ...
            return_state=True, #Pasamos tanto estado oculto como secuencia de estados
...
class Decoder(tf.keras.Model):
    def __init__(self, vocab_size, embedding_dim, dec_units, batch_sz):
        ...
        if AttentionType == "Bahdanau":
            ...
            self.W = tf.keras.layers.Dense(2 * dec_units)
    ...
    def call(self, x, hidden, enc_output):
        ...
        if AttentionType == "Bahdanau":
            ...
            output, state = self.gru(x, initial_state=hidden) #Comprobar con teoría
...
).shuffle(BUFFER_SIZE) #Mezcla los elementos del dataset
...
def loss_function(real, pred):
    mask = tf.math.logical_not(tf.math.equal(real, 0)) #masks '<pad>'
...
if CHECKPOINT == True:
    ...
    ckpt_manager = tf.train.CheckpointManager(ckpt, checkpoint_path, max_to_keep=2) #Guarda 1 checkpoint por tipo de Attention
