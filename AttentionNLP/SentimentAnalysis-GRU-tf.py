#
# Sentiment Analysis using RNN(GRU)
#
...
class SentimentAnalysis(tf.keras.Model):
    def __init__(self, hidden_units, output_units, vocab_size, embedding_dim):
        ...
        self.gru = tf.keras.layers.GRU(
            ...
            return_sequences=False, #Devuelve un único output
            return_state=False, #Cada estado oculto no devuelve nada
        )
        self.dense = tf.keras.layers.Dense(output_units, activation="sigmoid")

    def call(self, x):
        x = self.embedding(x)
        context_vector = self.gru(x)
